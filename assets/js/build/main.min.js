...
var Prism = function() {
    var lang = /\blang(?:uage)?-(?!\*)(\w+)\b/i, _ = self.Prism = {
        util: {
        ...
        highlightAll: function(async, callback) {
            for (var element, elements = document.querySelectorAll('code[class*="language-"], [class*="language-"] code, code[class*="lang-"], [class*="lang-"] code'), i = 0; element = elements[i++]; ) _.highlightElement(element, async === !0, callback);
        },
        highlightElement: function(element, async, callback) {
            ...
            grammar) {
                ...
                if (code) {
                    ...
                    if (_.hooks.run("before-highlight", env), async && self.Worker) {
                        var worker = new Worker(_.filename);
                        worker.onmessage = function(evt) {
                            env.highlightedCode = Token.stringify(JSON.parse(evt.data), language), _.hooks.run("before-insert", env), 
                            env.element.innerHTML = env.highlightedCode, callback && callback.call(env.element), 
                    ...
                    for (var i = 0; i < strarr.length; i++) {
                        ...
                        if (!(str instanceof Token)) {
                            ...
                            if (match) {
                                ...
                                var from = match.index - 1 + lookbehindLength, match = match[0].slice(lookbehindLength), len = match.length, to = from + len, before = str.slice(0, from + 1), after = str.slice(to + 1), args = [ i, 1 ];
    ...
    if (Token.stringify = function(o, language, parent) {
        ...
        if ("[object Array]" == Object.prototype.toString.call(o)) return o.map(function(element) {
    ...
    }, !self.document) return self.addEventListener ? (self.addEventListener("message", function(evt) {
        var message = JSON.parse(evt.data), lang = message.language, code = message.code;
        self.postMessage(JSON.stringify(_.util.encode(_.tokenize(code, _.languages[lang])))), 
        self.close();
    }, !1), self.Prism) : self.Prism;
    var script = document.getElementsByTagName("script");
    return script = script[script.length - 1], script && (_.filename = script.src, document.addEventListener && !script.hasAttribute("data-manual") && document.addEventListener("DOMContentLoaded", _.highlightAll)), 
    self.Prism;
}();
...
"undefined" != typeof module && module.exports && (module.exports = Prism), Prism.languages.markup = {
...
}), Prism.languages.markup && (Prism.hooks.add("before-highlight", function(env) {
    "php" === env.language && (env.tokenStack = [], env.backupCode = env.code, env.code = env.code.replace(/(?:<\?php|<\?)[\w\W]*?(?:\?>)/gi, function(match) {
        return env.tokenStack.push(match), "{{{PHP" + env.tokenStack.length + "}}}";
    }));
}), Prism.hooks.add("before-insert", function(env) {
    "php" === env.language && (env.code = env.backupCode, delete env.backupCode);
}), Prism.hooks.add("after-highlight", function(env) {
    if ("php" === env.language) {
        for (var t, i = 0; t = env.tokenStack[i]; i++) env.highlightedCode = env.highlightedCode.replace("{{{PHP" + (i + 1) + "}}}", Prism.highlight(t, env.grammar, "php"));
        env.element.innerHTML = env.highlightedCode;
    }
}), Prism.hooks.add("wrap", function(env) {
    "php" === env.language && "markup" === env.type && (env.content = env.content.replace(/(\{\{\{PHP[0-9]+\}\}\})/g, '<span class="token php">$1</span>'));
    ...
    function hasClass(element, className) {
        return className = " " + className + " ", (" " + element.className + " ").replace(/[\n\t]/g, " ").indexOf(className) > -1;
    }
    function highlightLines(pre, lines, classes) {
        for (var range, ranges = lines.replace(/\s+/g, "").split(","), offset = +pre.getAttribute("data-line-offset") || 0, lineHeight = parseFloat(getComputedStyle(pre).lineHeight), i = 0; range = ranges[i++]; ) {
            range = range.split("-");
            var start = +range[0], end = +range[1] || start, line = document.createElement("div");
            line.textContent = Array(end - start + 2).join(" \r\n"), line.className = (classes || "") + " line-highlight", 
            hasClass(pre, "line-numbers") || (line.setAttribute("data-start", start), end > start && line.setAttribute("data-end", end)), 
            line.style.top = (start - offset - 1) * lineHeight + "px", hasClass(pre, "line-numbers") ? pre.appendChild(line) : (pre.querySelector("code") || pre).appendChild(line);
        }
    }
    function applyHash() {
        ...
        if (range && !document.getElementById(hash)) {
            ...
            pre && (pre.hasAttribute("data-line") || pre.setAttribute("data-line", ""), highlightLines(pre, range, "temporary "), 
            document.querySelector(".temporary.line-highlight").scrollIntoView());
        }
    }
    if (window.Prism) {
        var fakeTimer = (crlf = /\r?\n|\r/g, 0);
        Prism.hooks.add("after-highlight", function(env) {
            var pre = env.element.parentNode, lines = pre && pre.getAttribute("data-line");
            pre && lines && /pre/i.test(pre.nodeName) && (clearTimeout(fakeTimer), $$(".line-highlight", pre).forEach(function(line) {
                line.parentNode.removeChild(line);
            }), highlightLines(pre, lines), fakeTimer = setTimeout(applyHash, 1));
        }), addEventListener("hashchange", applyHash);
    }
}(), Prism.hooks.add("after-highlight", function(env) {
    var pre = env.element.parentNode;
    if (pre && /pre/i.test(pre.nodeName) && -1 !== pre.className.indexOf("line-numbers")) {
        var lineNumbersWrapper, linesNum = 1 + env.code.split("\n").length;
        lines = new Array(linesNum), lines = lines.join("<span></span>"), lineNumbersWrapper = document.createElement("span"), 
        lineNumbersWrapper.className = "line-numbers-rows", lineNumbersWrapper.innerHTML = lines, 
        pre.hasAttribute("data-start") && (pre.style.counterReset = "linenumber " + (parseInt(pre.getAttribute("data-start"), 10) - 1)), 
